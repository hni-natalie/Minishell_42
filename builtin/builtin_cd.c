...
/*   Updated: 2025/02/18 11:35:35 by rraja-az         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* static int	home_directory(t_shell *shell)
{
	...
	if (oldpwd)
		...
		update_env("OLDPWD", get_directory("PWD", shell), true, shell);
	home = get_directory("HOME", shell);
	if (!home)
	{
		printf("minishell: cd: HOME not set\n");
...
/*
	...
	** getcwd(NULL, 0) ; auto determine the required buffer size > mallocs > store value
		> needs freeing
	** getcwd(char *buf, size_t size) > no need to free 
*/
/* static int prev_directory(t_shell *shell)
{
	char	*oldpwd;
	...
	if (cwd)
	{
		//update_env("OLDPWD", cwd, false, shell);
		printf("%s\n", cwd);
		free(cwd);
	}
	return (update_pwd(shell)); 
